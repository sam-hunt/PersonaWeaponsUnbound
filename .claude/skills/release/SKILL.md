...
folder from a whitelist of runtime file types, so the explicit clean above
only clears build artifacts (obj/bin) - no separate deploy step is needed.
...
refresh in step 2; if it reports that, **stop and ask the user** to close
the client and rerun):

```bash
python3 Scripts/integration-smoke-test.py
```

- Boots the freshly deployed build once on a pinned list of PWU plus its
  integration mods (VEF, VPWE) and family siblings (graphical boot,
  ~1-2 min, auto-quits), then classifies every Player.log error by origin
  and fails on anything attributed to PWU or an integration seam. This is
  the only automated coverage the conditional integration patches get; it
  exists because the BetterTradersGuild v1.1.0 CWTL regression was invisible
  without an integration mod active (see CLAUDE.md's Tests section).
...
- Rebuild (`dotnet build PersonaWeaponsUnbound.sln -c Release`) so the
