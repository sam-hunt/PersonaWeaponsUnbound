...
- Current DefInjected targets and their translatable fields (checked against
  ...
  - DefInjected keys are `DefName.field` paths
    (`PWU_BladelinkCustomization.description`) — the checker errors on any
    uncovered expected key (the `required` subset of the
    `Scripts/expected-injections.json` sidecar — see the externally-sourced
    bullet below) and on cross-language drift (a key translated in one
...
- **Some translatable fields can exist nowhere in a mod's XML** — text
  ...
  Reading `Defs/` cannot find them, so the expected key set is not derived
  from `Defs/` at all: it is `Scripts/expected-injections.json`, a checked-in
  dump of every injection point the *live* game sees for this mod,
  regenerated by `Scripts/refresh-translation-expectations.py` (launches the
  game with the `../L10nProbe` dev mod, which calls the game's own walker).
  The checker enforces the sidecar's `required` subset per language and fails
  on stale expectations, so new content of *any* shape forces a regen rather
  than a hand-maintained manifest row and there is no in-game-report backstop
  to remember. This repo's defs are all self-contained, so today the sidecar
  holds only their own `label`/`description`/`jobString`/`reportString` — but
