...
- **Unlock-list gating for the persona-core recipe** (`enablePersonaCoreRecipe`, default false): `AvailableNow` alone isn't enough for this one. `Verse.ResearchProjectDef.UnlockedDefs` builds its list from every recipe whose `researchPrerequisite` is the project, so the core showed up under machine persuasion's "Unlocks" even with the toggle off — and that single property feeds four surfaces: the research tab's Unlocks row (`ResearchPrerequisitesUtility`), the research-completed dialog (`ResearchManager.FinishProject`), `ResearchProjectDef.InfoCardHyperlinks`, and Ideology's missing-meme warning. Fixed with a postfix on that getter (`ResearchProjectDef_UnlockedDefs_Patch`), chosen over stripping `researchPrerequisite` off the recipe def so the entry still shows correctly when the recipe _is_ enabled. Three constraints it has to respect: the getter returns its own `cachedUnlockedDefs` **by reference** and vanilla never rebuilds it, so the filter returns a copy (an in-place `RemoveAll`, as used for `VisibleResearchProjects`, would be permanent and survive re-enabling); `cachedHyperlinks` and `MainTabWindow_Research.cachedUnlockedDefsGroupedByPrerequisites` are built from the filtered list and need reflective busting on settings write; and a startup scan skips the filter entirely when another mod also produces `AIPersonaCore` off machine persuasion, so we never hide someone else's unlock.
