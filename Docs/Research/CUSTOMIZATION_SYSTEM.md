...
> **Fork note (2026-07):** This doc was researched for the UWU (Odyssey unique weapons) era of this codebase. The styling-station, preview, job-queuing, float-menu, and implementation-architecture analysis remains the live architectural reference for Persona Weapons Unbound. The Odyssey-specific sections (DLC Gating Analysis, Unique Weapon API Reference, and the `CompUniqueWeapon`-based Design Mapping) were retired once the persona dialog stabilized — see git history for the removed material, and `Docs/Research/BLADELINK_WEAPONS.md` / `Docs/Specs/PERSONA_FORK.md` for the Royalty bladelink equivalents.
...
For weapon customization, we'd need to preview **trait effects** (stat changes, name, color, graphic). We should NOT mutate the weapon's customization comp directly. Instead:
...
7. JobDriver_CustomizeWeapon executes:
   ...
   d. WORK LOOP:
      ...
      iii. Do:
           ...
           - Add/remove trait on the customization comp
