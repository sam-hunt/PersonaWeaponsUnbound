...
namespace PersonaWeaponsUnbound.HaulPlanning
{
    ...
    public class HaulPlan
    {
        ...
        // should set HaulPlanExecutionStrategy.VanillaCarryOnly to opt into
        // the simple, vanilla-toil-only haul chain. Planners that pack
        // multiple pickups per trip with destination metadata (CarryTracker
        // for one stack + Inventory for the rest) must set
        // HaulPlanExecutionStrategy.UwuCarryInventoryHybrid.
    ...
    // Default VanillaCarryOnly matches the pre-Sweep behavior exactly so
    // plans missing this field (e.g. an unset literal) and old saves resume
    // on the bedrock path.
    ...
    // Default CarryTracker matches Sequential's one-stack-per-trip behavior so
    // existing planner output works unchanged.
