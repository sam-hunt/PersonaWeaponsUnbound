...
# cannot: fields inherited from VANILLA parents (a weapon's tools list,
# FactionBase's messageDefendersAttacking, InjuryBase's labelNounPretty) and
# C# field DEFAULTS never written in any XML (an ability comp's chargeNoun /
# cooldownGerund). This repo happens to have none of either today — its defs
# are all its own JobDef/RecipeDef/ResearchProjectDef with no vanilla parents
# — but that is something the sidecar reproves on every regen, not an
# assumption baked into this script.
# Unknown gap classes are impossible by structure: any
...
#   <PWU_ResetToDefaults>...</PWU_ResetToDefaults>
...
# DLCs the sidecar must have been generated with. Derived from About.xml's
# modDependencies (Harmony plus ludeon.rimworld.royalty) and a grep for
# MayRequire= across Defs/, which finds none: Royalty is the mod's whole
# subject matter and a hard dependency, so without it the defs do not load at
# all and the dump would be empty; nothing else is DLC-gated, so no other DLC
# can silently drop keys from a dump. Add a DLC here the moment a def
# MayRequire's it.
REQUIRED_DLCS = {"Royalty"}
...
def check_language(lang_dirs, english_keyed, sidecar, def_roots, report):
    ...
    # root is not a language of its own: a MayRequire-gated def's injections
    # (MayRequire is ignored on DefInjected, so a LoadFolders-gated
    # 1.6/Mods/<Mod Name>/Languages/<Lang>/ folder is the only way to gate
    # them) can only live under their own compat root, yet the sidecar would
    # still mark them required — checking that root standalone would demand
    # the whole Keyed set and every other def's injections from it. This repo
    # ships no compat roots today; the multi-root model exists so adding one
    # doesn't require re-deriving this.
    ...
    # sibling repo UMW's WeaponCategoryDef labels shipped missing in all its
    # languages).
