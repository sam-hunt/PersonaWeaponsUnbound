...
- Every translated entry carries the current English source in a comment
  directly above it, e.g. `<!-- EN: Customize persona: {0} -->` — this is how
  stale translations are detected when the English changes.
- Placeholders (`{0}`, `{1}`, ...) must match the English exactly.
- DefInjected folders use the def type's name (`JobDef`, `RecipeDef`,
  `ResearchProjectDef` — this mod currently defines no custom def types).
